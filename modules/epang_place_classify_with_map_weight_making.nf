...
workflow epang_place_classify_wf {
    ...
    // --- pplacer NBC classification path (uses map/weights) ---
    ClassifyDB_Prep( refpkg_tgz_f, sv_map_f )
    ClassifySV( refpkg_tgz_f, ClassifyDB_Prep.out, MergeJplaces.out, CombineAln_SV_refpkg.out )
    ClassifyMCC( ClassifySV.out, sv_weights_f )
    ranks_for_tables = Channel.from('species','genus','family','class','order','phylum')
    ClassifyTables( ranks_for_tables.combine( ClassifyMCC.out.mix( sv_map_f ) ) )

    emit:
        jplace_dedup = MergeJplaces.out
        taxonomy = Gappa_Extract_Taxonomy.out[0]
        classify_mcc = ClassifyMCC.out
}
process SvlToMapWeights {
    ...
    df = pd.read_csv("${svl_csv}")
    req = {"specimen","sv","count"}
    if not req.issubset(df.columns):
        raise SystemExit(f"svl is missing required columns: {req - set(df.columns)}")

    # representative specimen per SV (highest count)
    idx = df.groupby('sv')['count'].idxmax()
    sv_rep_sp = df.loc[idx].set_index('sv')['specimen'].to_dict()
    rep_pairs = set((sv, sp) for sv, sp in sv_rep_sp.items())

    # sp_sv id: representative gets plain sv, others get specimen__sv
    df['sp_sv'] = [ sv if (sv, sp) in rep_pairs else f"{sp}__{sv}" for sp, sv in zip(df['specimen'], df['sv']) ]

    # weights: (sv, sp_sv, count) headerless
    weights = df[['sv','sp_sv','count']].copy()
    weights = weights.groupby(['sv','sp_sv'], as_index=False)['count'].sum()
    weights.to_csv('sv_weights.csv', index=False, header=False)

    # map: (sp_sv, specimen) headerless
    m = df[['sp_sv','specimen']].drop_duplicates()
    m.to_csv('sv_sp_map.csv', index=False, header=False)
    """
}
...
workflow {
    ...
    if (
        ...
        SvlToMapWeights( file(params.svl_csv) )
        map_f = SvlToMapWeights.out.sv_map
        weights_f = SvlToMapWeights.out.sv_weights
    }
    else if  (
        (params.sv_fasta != null) &&
        (params.sv_long != null )
        ...
        SvlToMapWeights( file(params.sv_long) )
        map_f = SvlToMapWeights.out.sv_map
        weights_f = SvlToMapWeights.out.sv_weights
    }
    else if (
